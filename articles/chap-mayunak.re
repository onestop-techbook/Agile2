...
アジャイルに初めて取り組んだとき、期待していたような成果がすぐに出ず、焦りを感じたことはありませんか。プロジェクト序盤から問題が噴出し、むしろ進捗が遅く感じたことはないでしょうか。何より、周りから「アジャイルって速いんじゃなかったの？」などの批判を受け、チーム全体が意気消沈してしまったことはありませんでしたか。
...
POが緊張した面持ちで、デモを始めようとしています。スプリント1のデモは特に見せられるものがなく、気まずい雰囲気で終わってしまいました。デザイナーと開発者も参加していますが、何か揉めているように小声で会話を交わしています。部屋にステークホルダーが入ってきました。デモが始まります。


まずは、今回のスプリントで取り組んだユーザーストーリーのデモです。アプリを立ち上げるといくつかのカード型UIが表示されました。カードの中に表示されるメッセージはまだダミーのテキストで、見た目も簡素です。
...
静まり返った時間が流れます。デザイナーが、しどろもどろになりながらデザインプロトタイプを見せ始めます。
...
これは、私が関わった複数のプロジェクトで体験したデモの様子を組み合わせたものです。デザイナーは、デザインに必要な時間が十分与えられていないと感じ、まだ自分の中でも納得がいっていないものを見せ、それに対して否定的な意見がくることに不満でした。開発者は、デザインに時間がかかりすぎていると感じ、開発を進められないまま時間がすぎていくことに苛立ちを感じていました。POは、スプリント内で終わると踏んでいたストーリーが終わらず、焦りを感じ、デザインと開発者のすれちがいに気づいていませんでした。ステークホルダーは、アジャイルに「とにかくスピードが上がり、ものすごいプロダクトがパッと出てくるもの」という印象を持っていました。
...
また、プロダクト開発がチームスポーツであること、私たちが人間であるということも、不確実性を上げる要素です。特に、新しくできたばかりのチームでは、お互いのペースを掴むまでの試行錯誤も不確実性の要素になるでしょう。


//image[image1][プロダクトとプロダクトを構築する方法における不確実性の例][scale=1.0]


逆に、アジャイルな計画づくりがうまくいくためには、この不確実性を受け入れることが大切な第一歩となります。前出の『アジャイルな見積りと計画づくり』では、最後の章で架空のチームが登場し、さまざまな課題に直面しながら、計画を修正していく様子が描かれています。その様子を見ていると、チームがプロダクトについてだけでなく、自分たちについても少しずつ学び、チームワークの質を上げていく様子を見ることができます。
...
デザインと開発の協業経験の有無によっても、不安度は変わります。今まで「デザイン仕様」を受け取ってから開発を始めていた場合、真っ白な状態からデザインが始まることに不安を感じる人は多いでしょう。


もちろん、リリース用の開発が始まる前に「ディスカバリー」を行い、ある程度コンセプト作りをしているケースがあるかもしれません。しかし、おおまかな方向性が定まっていたとしても、デザインという作業は発散と収束を繰り返します。しかも一度ではなく、視点を変えながら何度もその過程を繰り返すことで、最終的なソリューションに落ち着きます。つまり、発散させている間は常に「何が正しいのかわからない」という状態が続くのです。
...
鍵となるのは、いかに素早くプロダクトチームとして成長し、お互いのリズムを掴むか。初めのうちに疑問点や不協和音を見ないフリをしてしまうと、あとあと自分たちに戻ってくることになります。つまり、不安になることが起こった場合、多少時間がかかったとしても、未来の自分たちを助けるつもりで対処するほうがよいのです。
...
「小さく始め、改善させながら積み重ねていく」ことは、チーム自体の成長にもいえることです。まだ取り組み始めたばかりのチームが、フレームワークを試してみてもうまくいかなくて、不安に思うことは当たり前のことです。誰かが不安を感じていたら、それはチームがステップアップするチャンスです。その理由に注目して、どうしたら解消できそうか考えてみてください。
