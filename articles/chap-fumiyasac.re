...
Androidでは、図解で示す様なAnimation表現を伴った便利なかたちのコンポーネントが、純正コンポーネントとして提供されている場合がありますが、iOSで類似した表現を実現する場合は、自前で準備する場合や近しい振る舞いを実現可能なOSSを利用する方針をとる場合もあるかと思います。このように、技術的には実現可能であったとしても、片方のOSではあまりお目にかからないUI表現であるケースもあるので、両OSでのガイドラインはもちろん、実装の参考アプリケーションにおいても、iOS／Android版を見比べながら調査を進めると、よりよい判断や選択ができると思います。OSSを利用する方針をとる場合には、特に要件にあわせた柔軟なカスタマイズができる余地があるか、そして頻繁に更新や改善が実施されているかという点に注目するとよいでしょう。
