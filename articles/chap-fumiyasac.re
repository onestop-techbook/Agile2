...
私はこれまで、iOSとAndroidの両方でモバイルアプリ開発を経験してきました。その中で、デザインを基に実装の難易度を見積もることが、プロジェクトの成功においていかに重要であるかを強く実感しています。iOSとAndroidは、それぞれ異なるデザインガイドラインや技術的な要件を持っており、同じデザインを実装する場合でも、プラットフォームごとに異なるアプローチが必要です。
...
UIコンポーネントの選択は、開発の難易度に大きな影響を与えます。標準のUIコンポーネントで実現できる場合は、開発の手間が軽減されますが、カスタムUIコンポーネントが必要な場合は、実装とテストに時間がかかります。また、アニメーションやレイアウトの複雑さも、プロジェクトの進行を左右します。シンプルな遷移や配置よりも、複雑なインタラクションや階層構造を持つデザインの方が当然必要な工数は多くかかる事になります。
...
「デザインから逆算して難易度を見積もるための観点」がアジャイル開発において重要となる理由は、アジャイル開発の特性や目指す成果と密接に関係するからと考えています。
 ...
 * @<fn>{iosdc2021_pamphlet_manuscript}円滑なUI＆機能実装やデザイナーとの共同作業を進めるために心がけてきた事
 * @<fn>{iosdc2021_pamphlet_manuscript}複雑なUI実装の壁を越えるための考え方事例紹介 (iOS/Android間で実装を合わせるヒント)
