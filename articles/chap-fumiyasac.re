...
筆者自身はモバイルアプリケーション開発の中でも、特にUI実装に関連する分野への関心が高いです。この点については、20代のときにデザイナーとしてのキャリアを歩んだことも関係していると思います。ゆえにモバイルアプリケーション開発において、UI実装の観点から見る難易度の見積もりは、プロジェクトの成功に直結する重要な要素であると考えています。
...
一見すると、同様な見た目や振る舞いをするようなUIに思えるものであっても、いざ実装を進めていくと、必要な工数や手間が、iOSとAndroidで大きく食いちがうような場合も珍しくありません。それゆえに、両方のUI表現を合わせる方針をとるよりも、それぞれのデザインガイドラインに沿うかたちにするほうが、かえってシンプルになる場合もあります。
...
最初は小さな単位でもかまいませんので、UI実装関連の手がかりとなる情報を整理したうえで共有したり、チームメンバー同士で実際の成果物に対して考察をする機会を設けたりしながら、チームの共通理解と連携強化を図るような活動を実践することが、まずは大切だと考えています。想定以上に考慮すべき点や実装を進めるうえでの懸案事項を見えるかたちにして共有することは、チームメンバー内での認識を合わせることにつながり、正確な難易度の見積もりを実施するときの貴重な知見となります。実現可能性を考えるための判断材料となる情報源を、自分ないしはチーム全体で持つことは、エンジニアのみならず、それ以外の職種の方にとっても有意義だと思います。
...
平素の業務以外でもUI実装に関するサンプルや解説記事を投稿したり、iOS／Androidアプリケーション開発での勉強会で登壇しています。
アイデアを練ったり、設計のためのメモや図解を作るときはもっぱら手書きで描くことが多いです。
