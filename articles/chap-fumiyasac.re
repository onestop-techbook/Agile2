...
〓本節〓では、デザインから逆算して難易度を見積もるときに考慮すべき重要な観点について、筆者の経験を踏まえて解説します。これらの観点をおさえることで、開発チームがデザインと技術的実現性のバランスを取りながら、効率的かつ高品質なアプリを開発するためのヒントを提供できればと思います。
...
筆者自身はモバイルアプリ開発の中でも、特にUI実装に関連する分野への関心が高いです。この点については、20代のときにデザイナーとしてのキャリアを歩んだことも関係していると思います。ゆえにモバイルアプリ開発において、UI実装の観点から見る難易度の見積もりは、プロジェクトの成功に直結する重要な要素であると考えています。

と同時にモバイルアプリ開発において、iOS・AndroidのUI実装をそれぞれの特徴を踏まえた上で高い精度で実装難易度を見積もる事は、これまでに開発者が実践してきた試行錯誤や経験の積み重ねに左右されやすい点や、場合によっては必ずしも1通りの正解に行き着くとは限らない点を踏まえると、想定する以上に難しいと言えます。

一見すると、同様な見た目や振る舞いをする様なUIに思える様なものであっても、いざ実装を進めていくと必要な工数や手間が、iOSとAndroidで大きく食い違う様な場合も珍しくありません。それ故に、両方のUI表現を合わせる方針を取るよりも、それぞれのデザインガイドラインに沿う形にする方が却ってシンプルになる場合もあります。

モバイルアプリ開発における、UI実装の難易度をより正確に見積もるために持っておくと良い観点を整理すると、この様になると考えています。
...
〓本節〓で提示するのはほんの一例にはなりますが、次の観点から判断をするときの参考になれば嬉しく思います。
...
最初は小さな単位でも構いませんので、UI実装関連の手掛かりとなる情報を整理した上で共有したり、チームメンバー同士で実際の成果物に対する考察をする機会を設けたりしながら、チームの共通理解と連携強化を図る様な活動を実践する事が、まずは大切だと考えています。想定以上に考慮すべき点や実装を進める上での懸案事項を見える形にして共有する事は、チームメンバー内での認識を合わせる事に繋がり、正確な難易度見積もりを実施する際の貴重な知見となります。実現可能性を考えるための判断材料となる情報源を、自分ないしはチーム全体で持つ事はエンジニアのみならず、それ以外の職種の方にとっても有意義だと思います。
...
こちらは、写真アスペクト比率を考慮したGrid表示を構築する際の例になります。AndroidでRecyclerViewを利用したレイアウトでは「StaggeredGridLayoutManager」を利用する事で比較的シンプルに実現できますが、iOSでUICollectionViewを利用するレイアウトにおいては、表示時に算出したアスペクト比率を考慮してセル要素のサイズを決定する必要があります。こちらは、著名なモバイルアプリでも良くお目にかかる機会も多い表現ではあるものの、一方のOSでは比較的簡単に実現できるため、同様の工数でもう一方のOSでの実装を進めると想定以上の工数を要してしまう典型的な事例の1つと言えます。

//image[001_difference_ios_android1][iOS/Android間で考え方が異なる事例（1）][scale=0.8]

こちらの表現についても、様々なメディアや読み物系のモバイルアプリで良くお目にかかる事例だと思います。Androidでは「純正コンポーネントで提供されているViewPagerとTabLayoutの組み合わせ」を利用して実現する方針を取り、iOSでは「UIPageViewControllerとUIScrollView(UICollectionView)の組み合わせ」で実現する場合が多いです。大まかな方針自体は割と近しいイメージではありますが、切り替え時の表現をカスタマイズしたい場合や、タブ表示要素部分を無限循環する形を実現したい場合では、想定以上に難易度が膨れ上がる事や、純正コンポーネントだけでは表現できない事もあるので注意が必要です。

//image[002_difference_ios_android2][iOS/Android間で考え方が異なる事例（2）][scale=0.8]

==== 2. プラットフォームで提供しているUI構成要素に関する例

Androidでは純正コンポーネントとして提供されていますが、iOSで類似した表現を実現する場合は、自前で準備する場合や近しい振る舞いを実現可能なOSSを利用する方針を取る場合もあるかと思います。この様に技術的には実現可能であったとしても片方のOSではあまりお目にかからないUI表現であるケースもあるので、両OSでのガイドラインはもちろん、実装の参考アプリにおいてもiOS/Android版を見比べながら調査を進めるとより良い判断や選択ができると思います。OSSを利用する方針を取る場合には、特に要件に合わせた柔軟なカスタマイズができる余地があるか、そして頻繁に更新や改善が実施されているかという点に注目すると良いでしょう。

//image[003_example_only_android_component][iOSでは用意されていないがAndroidでは用意されている表現例][scale=0.8]

==== 3. 宣言的UIを例にした考え方を合わせられる可能性があり得る例

こちらは、前述したGrid表現と少し似ていますが、iOSでUIKitを利用する場合は「UICollectionViewCompositionalLayout」等を利用して複雑なレイアウトを構築する必要があったり、AndroidでAndroid Viewを利用する場合は「StaggeredGridLayoutManager」をそのまま利用するだけでは実現できない構造になります。この様な一筋縄ではいかない様なレイアウトも宣言的UIの考え方に当てはめると、実はシンプルに実装を進められる場合もあります。iOSではSwiftUIを利用し、AndroidではJetpack Composeを利用する想定ではあるものの、意外とレイアウト構築に対する基本的な考え方は近いことがわかるかと思います。

//image[004_example_decretive_component][複雑な要素をともなう画面を宣言的UIで考え方を合わせる][scale=0.8]

==== 4. 端末固有の機能を考える場合の例

カメラ・プッシュ通知・音や動画に関連する処理は、モバイルアプリ内の機能でも良く利用される反面、iOS/Android間における考え方や端末による差異が出やすいものになります。ここで紹介しているのは、動画プレイヤー機能を有するモバイルアプリにおいて、バックグラウンド状態になった場合においては音声のみ再生を継続する様な形を実現する際の事例になります。同様な機能やユーザー体験を提供するにあたり、実装方針はもとより個々の機能を実現するにあたり必要な知識や考え方の相違点や特徴を捉える事が必要になってきます。
