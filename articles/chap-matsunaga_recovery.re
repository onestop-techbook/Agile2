...
では、図1において、Investmentはどれで、Returnはどれでしょう？Returnというのはその活動の成果として得られる利益などを指すのですから、この図で言えば”Return”は一番右の「アウトカム」だけとなります。わかりますか？つまりみなさんが作っているソフトウェアシステムは、作って納入するだけで価値があるわけではなく、運用されて、何らかの効果が得られて初めて”Return”、すなわち価値となる得るわけです。ソフトウェアシステムは、それ単体ではInvestment、つまり投資にすぎないということです。ええ～～そんなあ～。
...
テキトー意訳すると、「やらなくても良いことを最大化する技術が不可欠です」とかそんな意味です。やらないことの最大化、というのは、言い換えると、やることを最小限にしろということです。これは、上述のInvestmentを最小化せよという話と符合します。そのためにはシンプルさが必要だとアジャイル宣言は説いているわけです。一方で、いくらやることを減らすと言っても価値まで減らしてはいけないので、そのためによりシンプルなソリューションを考え、スプリントゴールで策定した価値を毀損することなく、やることを最小化すべしと言っているわけです。
 ...
 * シンプルな仕事をすることで、価値を毀損せずに量や時間を押さえることが可能となる。 
 * リカバリーのために残業をするのは悪手。
