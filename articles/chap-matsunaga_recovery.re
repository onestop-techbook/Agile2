...
では、〓図7.1〓において、Investmentはどれで、Returnはどれでしょう。Returnというのはその活動の成果として得られる利益などを指すのですから、この図でいえば「Return」は一番右の「アウトカム」だけとなります。わかりますか。つまりみなさんが作っているソフトウェアシステムは、作って納入するだけで価値があるわけではなく、運用されて、なんらかの効果が得られて、初めて「Return」、すなわち価値となり得るわけです。ソフトウェアシステムは、それ単体では「Investment」、つまり投資にすぎないということです。ええ～そんなあ～。
...
〓前節〓からわかるとおり、スプリントの目的は価値を生み出すことであり、ソフトウェアを作ることではありません。スクラムでは、スプリントで生み出されるこの「価値」をスプリントゴールとして設定するわけです。つまり、スプリントの目的は、より多くの機能を作るとか、より長時間働くとか、計画された機能をすべて完成させるとかではなく、価値を届けることだといえます。スプリントゴールとは、スプリントの「Why」です。「『なぜ』そのスプリントをやるのか？」です。つまり、スプリントゴールとは、そのスプリントが生み出そうとしている「価値」だということです。

ここで価値について考えてみます

筆者は、大好きなLiSA（歌手）さんがCatch the momentのリリースイベントで手渡してくれたサイン入りポストカードを持っています（2枚！）。彼女はこのサインをものの数秒で書き上げます。これなんて、売れば数万円ぐらいの値がついてもおかしくありません（絶対にそんなことはしませんが）。数秒で数万円、例えば5秒で5万円とすると、1分で60万円（！）の価値を生むのです。ですがそんなことよりも、彼女とほんの短時間でも直接目を合わせて、会話をしながら手渡してもらったという思い出が詰まっていて、実際のところPricelessです。つまり、価値というのは、単純に作業時間や作業量とは単純にリニアに変換できるものではないということです。
...
こういう考え方だと、遅延が出てしまった場合は、残業をしてリカバリーするしかなくなってしまいます。残業は最悪です。残業は、何か問題があったときに、安易にその場しのぎのリカバリーをする方法です。こういった方法が恒常化しているチームは、問題（この場合は遅延）の根本的な原因にリーチしようとしないので、改善していかないのです。アジャイル宣言にある「サステナブル・ペース」にも反しますね。なので、生産性が上がらないといっているチームのコーチに入ったときに、私が最初にコーチすることは、残業を禁止にすることだったりします。こうすることで、彼らはいかに残業をせずに問題を解決するかを考えるようになります。
