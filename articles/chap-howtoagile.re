...
例えば、プロジェクトマネジメントの知識体系ガイドである「プロジェクトマネジメント知識体系ガイド（PMBOK(R)ガイド）」@<fn>{pmbok}を見てみると、2017年に発行された第６版で「アジャイル」の内容が初めて本文に組み込まれました。さらに、2021年に発行された第７版では、「価値を提供する」ことに重きを置いた、アジャイル的なプロジェクトマネジメントを行うための知識体系へと大きく変貌を遂げました。
...
なぜこれらのプロジェクト管理手法がソフトウェア開発の管理手法として適した手法であると考えられたのでしょう。建設分野・製造分野（特にオーダーメイド品生産）の開発管理手法は「毎回異なるものを作り、前と丸々同じものを作ることはほぼ無い」という点でソフトウェア開発手法と非常に近しかったためです。当時現役のITエンジニアだった諸先輩にこの頃の話をうかがったときにも、「開発管理手法が確立していなかった当時のソフトウェア開発（およびそのプロジェクト管理手法）からすると非常に革新的だった」とうかがっています。
...
しかし、これらの開発手法はおおむね「開発者は顧客が要求することを最初からすべて理解している」「開発者は顧客にこの先要求されることを最初からすべて予測している」といった@<b>{前提が成立すればうまくいく}、@<b>{これからの変化にも耐えられる}という点に基づいており、「最終の完成形のソフトウェアを作りきればその後は弄らない（弄りたくない）」という認識に立ちます。言い換えれば、@<b>{仕様書が過不足なく完璧に作られている}、といった、現実的には不可能なことが必要条件となっています。しかし、未来は予測できない不確実なものです。

また、これらの従来型の開発手法でも一応、状況の変化などは考慮しています。ただし、その考え方は、「状況の合わない部分が出ることはあって、そのときに小規模な改修はするが、基本は大きく合わなくなれば丸ごと作り直せば良い」という考えに立つものです。
...
そして、この課題に対する一つの答えである@<b>{アジャイル的な開発プロセス}が示されるにはもう少し時間が必要だったのでした。
...
//footnote[SCRUMbook][一応古本であれば入手は可能です。成立の経緯などは書籍に詳しいので、興味のある方は入手して読んでみても良いかもしれません。]
...
なぜなら、「価値」がズレてしまっても駄目だし、「プラクティス」だけをなぞってもうまく行かないもので、結局それをつなぐための「原則」がしっかりしている必要があります。さらにこれら３つの「価値」「原則」「プラクティス」を一体で使わないと、うまくいかない（アジャイルとして本来あるかたちになれない）からです。
...
スクラムやXPが本が出て一般的になったあと、その真意が掴み切れず「アジャイル開発」を銘打っているが実態はそうではなかった……　というプロジェクトが多く発生したようです。それに対するアンサーやガイドを兼ねたものでもあるのですが、2001年に「アジャイルソフトウェア開発宣言（Agile Manifest）」@<fn>{agilemanifesto}が世に出ます。
...
実際、XPは革新的であった。 ……というよりはあまりに革新的すぎて、国内でも話題になったときは大きな衝撃を伴うものでした。

XPが話題になった当時、筆者は大手のSIerでシステム開発に従事していました。XPが話題になったときに、「いってることはわかるけど社内でコレ適用できんの？」という話題で盛り上がった事を今でも鮮明に覚えています。

なお、SIerが当時一般的に行っていた受諾開発とXPは基本的に食い合わせが悪く、筆者は結局その会社ではアジャイル的なプロジェクトに従事することは無かったのでした。
...
もう１つは、サイボウズ株式会社が出しているkintone SIGNPOST@<fn>{kintonesingpost}です。
