...
例えば、プロジェクトマネジメントの知識体系ガイドである「プロジェクトマネジメント知識体系ガイド(PMBOK(R)ガイド」@<fn>{pmbok}をみてみると、2017年に発行された第６版で「アジャイル」の内容が初めて本文に組み込まれました。さらに、2021年に発行された第７版では、「価値を提供する」事に重きを置いた、アジャイル的なプロジェクトマネジメントを行うための知識体系へと大きく変貌を遂げました。
...
ここでは「ウォーターフォール型開発モデル」の興りについて挙げます。
...
その後、これらの取り組みが1995年のOOPSLAカンファレンスで共同発表されます。この内容は後に「アジャイルソフトウェア開発スクラム」(桐原書店. 2003)という書籍としてまとめられたことで、広く知られる事になります。
...
そしてこれらの実践におけるの知見などを踏まえ、1999 年Kent Beck により『Extreme Programming Explained: Embrace Change』(邦題：エクストリームプログラミング)という一冊の本@<fn>{xpbook}@<fn>{bookinfo}が世に出ました。
...
//footnote[basedon2Ed][基本的にすべてXP本第２版ベースで記載します。それぞれの内容については、ぜひ原典であるXP本を読んでみてください。]
...
スクラムやXPが本が出て一般的になった後、その真意が掴み切れず「アジャイル開発」を銘打っているが実態はそうではなかった……　というプロジェクトが多く発生したようです。それに対するアンサーやガイドを兼ねたものでもあるのですが、2001年に「アジャイルソフトウェア開発宣言(Agile Manifest)」@<fn>{agilemanifesto}が世に出ます。
...
=== アジャイル宣言の背後にある12の原則
アジャイル開発宣言に併せて、@<b>{12の原則}@<fn>{principles}も公表されました。これは次のようなものです。
...
XP本は、オーム社から日本語版が「エクストリームプログラミング」(2015)として、第２版が出ています。
