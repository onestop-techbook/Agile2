...
例えば、プロジェクトマネジメントの知識体系ガイドである「プロジェクトマネジメント知識体系ガイド（PMBOK(R)ガイド）」@<fn>{pmbok}を見てみると、2017年に発行された第6版で「アジャイル」の内容が初めて本文に組み込まれました。さらに、2021年に発行された第7版では、「価値を提供する」ことに重きを置いた、アジャイル的なプロジェクトマネジメントを行うための知識体系へと大きく変貌を遂げました。

//footnote[pmbok][https://www.pmi-japan.shop/shopdetail/000000000028/]

昨今では、ソフトウエア開発やプロジェクトマネジメント以外のいろいろな分野に波及しており、相変わらずアツい「アジャイル」ですが、そもそも「アジャイル」はどういった経緯で必要となり一般的になっていったのでしょうか？

この章では、その思想やあり方を手っ取り早く、かつ深く理解する為に「アジャイル」が発生するにいたったソフトウェア開発手法の発展の歴史を追うことで、アジャイルが求められるようになった背景について、まずは学んでいきましょう！

=== アジャイルが求められるようになった背景を爆速で理解しよう

ソフトウェア開発の黎明期（1970年頃）は国内外を問わず、情報システム産業全体の課題として、ソフトウェア開発の生産性やソフトウェア品質が非常に悪いことが大きな問題となっていました。この問題に対してはさまざまな開発手法が提案されました。具体的にはウォーターフォール型開発モデルや、構造化プログラミングといった、現在の開発手法にもつながる手法が提案され、またさまざまなプロジェクトで実践されてきました。

ここでは「ウォーターフォール型開発モデル」の興りについてあげます。
ソフトウェア開発の生産性やソフトウェア品質が悪いという課題に対し、当時（1970年頃）のITの専門家たちが参照したのが建設分野のプロジェクト管理手法です。これは、開発プロセスとしてソフトウェア開発に一番近いモデルで開発を行っていると考えられたためです。建築や製造業のプロジェクト管理手法をソフトウェア開発に適用したのが、最初のソフトウェア開発手法である、ウォーターフォール型開発モデルです。

なぜこれらのプロジェクト管理手法がソフトウェア開発の管理手法として適した手法であると考えられたのでしょう。建設分野・製造分野（特にオーダーメイド品生産）の開発管理手法は「毎回異なるものを作り、前と丸々同じものを作ることはほぼない」という点でソフトウェア開発手法と非常に近しかったためです。当時現役のITエンジニアだった諸先輩にこの頃の話をうかがったときにも、「開発管理手法が確立していなかった当時のソフトウェア開発（およびそのプロジェクト管理手法）からすると非常に革新的だった」とうかがっています。
...
//footnote[piason1][ソフトウェア工学-理論と実践, Shari Lawrence Pfleeger, 桐原書店, 2001 から引用]




またウォーターフォール型開発モデルが世に出たあと、これを発展させたり応用したさまざまな開発モデルが考案され、実際の開発手法として運用されました。
 ...
 * スパイラルモデル：計画、目標・代替案・制約の決定、代替案とリスクの評価、開発とテストの各工程を何周も繰り返しながら一周ごとに設計→開発→テスト　と段階を追って進めていくことでソフトウェア開発を行うもの

このようにさまざまな開発モデルが考案され、また実際にたくさんの開発プロジェクトでこれらの開発手法が運用され、さまざまなソフトウェアが開発されてきました。

しかし、これらの開発手法はおおむね「開発者は顧客が要求することを最初からすべて理解している」「開発者は顧客にこの先要求されることを最初からすべて予測している」といった@<b>{前提が成立すればうまくいく}、@<b>{これからの変化にも耐えられる}という点にもとづいており、「最終の完成形のソフトウェアを作りきればその後は弄らない（弄りたくない）」という認識に立ちます。言い換えれば、@<b>{仕様書が過不足なく完璧に作られている}、といった、現実的には不可能なことが必要条件となっています。しかし、未来は予測できない不確実なものです。

また、これらの従来型の開発手法でも一応、状況の変化などは考慮しています。ただし、その考え方は、「状況の合わない部分が出ることはあって、そのときに小規模な改修はするが、基本は大きく合わなくなれば丸ごと作り直せばよい」という考えに立つものです。

しかしながら、実際は、1998年に出た本でも
『伝統的な「ウォーターフォール」のアプローチで開発を行うことは、今日のシステムではもはや柔軟でもふさわしくもない。』@<fn>{piason1}
と記載されています。ここまであげた各種の開発手法が、「（特に、未来の）変化に耐えられる」か？という観点では大きな課題を持っているという事実は、当時のソフトウェア開発手法の共通の課題であった事がうかがえます。





そして、この課題に対する1つの答えである@<b>{アジャイル的な開発プロセス}が示されるにはもう少し時間が必要だったのでした。

=== スクラムが産声をあげる

ときは前後して1993年、ソフトウェア開発手法としての「スクラム」が産声を上げます。
これは、1986年に野中郁次郎博士と竹内弘高博士が、当時の日本の製造業における革新的な開発手法を分析し、「スクラム」と名づけて発表した論文@<fn>{scrunletter}が原典です。

//footnote[scrunletter][The New New Product Development Game https://hbr.org/1986/01/the-new-new-product-development-game 1986]

さらに、1993年にJeff Sutherland、John Scumniotales、Jeff McKennaの3名がソフトウェア開発に適用した設計・分析ツールを構築し、またときを同じくして、Ken Schwaberも自社でのソフトウェア開発にこの手法、@<b>{スクラム}を用いました。

その後、これらの取り組みが1995年のOOPSLAカンファレンスで共同発表されます。この内容はのちに「アジャイルソフトウェア開発スクラム」（桐原書店,2003）という書籍としてまとめられたことで、広く知られる事になります。
英語板はAgile Software Development with SCRUM（Series in Agile Software Development）（Prentice Hall,2001）
...
さらに、ほぼ同時期に、@<b>{XP(eXtreme Programming)}も産声を上げます。
...
そしてこれらの実践におけるの知見などを踏まえ、1999 年Kent Beck により『Extreme Programming Explained: Embrace Change』（邦題：エクストリームプログラミング）という1冊の本@<fn>{xpbook}@<fn>{bookinfo}が世に出ました。

//footnote[bookinfo][Kent Beck, Cynthia Andres, Addison-Wesley Professional, 1999]
//footnote[xpbook][以下、XP本と記載します。邦訳の第2版は、現在も普通に購入可能です。 https://www.ohmsha.co.jp/book/9784274217623/　また、前述したXPが初めて実践されたプロジェクトについての話も12章「はじまりの物語」に記載があります。]
...
//footnote[practice][XP本では、プラクティスを主要プラクティスと導出プラクティスに分けています。『主要プラクティス（Primary practice）は、あなたがほかにやっていることとは無関係に役に立つものである。すぐに改善につながり、どれからでも安全に始められる。導出プラクティス（corollary practicices）は、先に主要プラクティスを習得しておかなければ難しいだろう。プラクティスを組み合わせれば増幅効果が得られるので、できるだけ早くプラクティスを追加した方が有利である。』（Extreme Programming Explained: Embrace Change　2nd　Editionからの引用]
...
なぜなら、「価値」がズレてしまっても駄目だし、「プラクティス」だけをなぞってもうまく行かないもので、結局それをつなぐための「原則」がしっかりしている必要があります。さらにこれら3つの「価値」「原則」「プラクティス」を一体で使わないと、うまくいかない（アジャイルとして本来あるかたちになれない）からです。
...
スクラムやXPに関する本が出て、用語や手法（プラクティス）として一般的になったあと、しかしその真意が掴み切れず「アジャイル開発」を銘打っているが実態はそうではなかった……　というプロジェクトが多く発生したようです。それに対するアンサーやガイドを兼ねたものでもあるのですが、2001年に「アジャイルソフトウェア開発宣言（Agile Manifest）」@<fn>{agilemanifesto}が世に出ます。

//footnote[agilemanifesto][アジャイルソフトウエア開発宣言 https://agilemanifesto.org/iso/ja/manifesto.html]

ここでは、4つの価値とそれのもととなる12の原則が示されました。
...
また、スクラムについては2004年にはじめて、情報処理学会の研究報告@<fn>{scrumreport}があがっています。
...
当時の雑誌媒体についても2誌で情報を調べてみました。おおよそこの歴史を裏づける内容で、情報が出てきたのも同様の順でした。

 * @<b>{XP}

Software Design誌

2002年09月号の特別企画「これでわかった！ＸＰの使い方」で一般的な記事が出た

？2002年8月号からエクストリームプログラミングについての記載が出始め、
...
2005年08月号の特集記事「緊急レポート！　最新ソフトウェア開発手法事情」の中で、「4章　繰り返し型開発の落とし穴　失敗から学ぶ効果的運用」が最初でした。とはいえあまりガッツリした内容ではありませんでしたが。
...
このように、雑誌での取り上げられた時期としてはXPのほうが早いことが確認できました。日本でのアジャイルの興りは2002~3年頃に来た、XPブームがおそらく最初の大きな波であったことがわかりました。

====[column]XPがやって来た（黒船がやってきたのノリで）
実際、XPは革新的であった。 ……というよりはあまりに革新的すぎて、国内でも話題になったときは大きな衝撃をともなうものでした。

XPが話題になった当時、筆者は大手のSIerでシステム開発に従事していました。XPが話題になったときに、「いってることはわかるけど社内でコレ適用できんの？」という話題で盛り上がった事を今でも鮮明に覚えています。

なお、SIerが当時一般的に行っていた受諾開発とXPは基本的に食い合わせが悪く、筆者は結局その会社ではアジャイル的なプロジェクトに従事することはなかったのでした。

結局XPは当時（2002～3年ぐらい）としてはあまりに先進的すぎたため、いろいろなアプローチがバズワード化してしまったり……と、それはそれで当時はネタにもされたものではありました（特にSI）

「なぜか2人で1つのPC使ってコード書くらしいで」
「テストから先に書くの？」
...
なお、なぜこうなってしまうか……　という国内的な事情としては、アメリカの企業は基本的に自社システムは社内でシステムを内製するが、日本の企業はSIer（システムインテグレーター）にシステムを外注する　という業務システムに対する開発体制の大きな差があり、こういった差をXPでは埋められなかったという話はありそうだな……　と思っています。
...
前述の契機となった話のあと、Jeff Sutherlandはこれらを整理・構築し、2011年に「スクラムガイド」@<fn>{scrumguide}をまとめ上げます。
...
XP本は、オーム社から日本語版が「エクストリームプログラミング」（2015）として、第2版が出ています。
