...
例えば、プロジェクトマネジメントの知識体系ガイドである「プロジェクトマネジメント知識体系ガイド（PMBOK(R)ガイド）」@<fn>{pmbok}を見てみると、2017年に発行された第６版で「アジャイル」の内容が初めて本文に組み込まれました。さらに、2021年に発行された第７版では、「価値を提供する」事に重きを置いた、アジャイル的なプロジェクトマネジメントを行うための知識体系へと大きく変貌を遂げました。
...
なぜこれらのプロジェクト管理手法がソフトウェア開発の管理手法として適した手法であると考えられたのでしょう。建設分野・製造分野（特にオーダーメイド品生産）の開発管理手法は「毎回異なるものを作り、前と丸々同じものを作る事はほぼ無い」という点でソフトウェア開発手法と非常に近しかったためです。当時現役のITエンジニアだった諸先輩にこの頃の話を伺った際にも、「開発管理手法が確立していなかった当時のソフトウェア開発（およびそのプロジェクト管理手法）からすると非常に革新的だった」と伺っています。
...
またウォーターフォール型開発モデルが世に出たあと、これを発展させたり応用した様々な開発モデルが考案され、実際の開発手法として運用されました。
...
しかし、これらの開発手法は概ね「開発者は顧客が要求することを最初から全て理解している」「開発者は顧客にこの先要求されることを最初から全て予測している」といった@<b>{前提が成立すればうまくいく}、@<b>{これからの変化にも耐えられる}という点に基づいており、「最終の完成形のソフトウェアを作りきればその後は弄らない（弄りたくない）」という認識に立ちます。言い換えれば、@<b>{仕様書が過不足なく完璧に作られている}、といった、現実的には不可能なことが必要条件となっています。しかし、未来は予測できない不確実なものです。
...
これは、1986年に野中郁次郎博士と竹内弘高博士が、当時の日本の製造業における革新的な開発手法を分析し、「スクラム」と名付けて発表した論文@<fn>{scrunletter}が原典です。
...
更に、ほぼ同時期に、@<b>{XP(eXtreme Programming)}も産声を上げます。
...
なぜなら、「価値」がズレてしまっても駄目だし、「プラクティス」だけをなぞってもうまく行かないもので、結局それをつなぐための「原則」がしっかりしている必要があります。さらにこれら３つの「価値」「原則」「プラクティス」を一体で使わないと、うまくいかない（アジャイルとして本来ある形になれない）からです。
