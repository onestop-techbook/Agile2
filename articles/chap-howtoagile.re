...
昨今では、ソフトウエア開発やプロジェクトマネジメント以外のいろいろな分野に波及しており、相変わらずアツい「アジャイル」ですが、そもそも「アジャイル」はどういった経緯で必要となり、一般的になっていったのでしょうか？
...
ソフトウェア開発の黎明期（1970年頃）は国内外を問わず、情報システム産業全体の課題として、ソフトウェア開発の生産性やソフトウェア品質が非常に悪いことが大きな問題となっていました。この問題に対しては、さまざまな開発手法が提案されました。具体的には、ウォーターフォール型開発モデルや、構造化プログラミングといった、現在の開発手法にもつながる手法が提案され、さまざまなプロジェクトで実践されていきました。

ここでは「ウォーターフォール型開発モデル」の興りについてあげます。
ソフトウェア開発の生産性やソフトウェア品質が悪いという課題に対し、当時（1970年頃）のITの専門家たちが参照したのが建設分野のプロジェクト管理手法です。これは、開発プロセスとしてソフトウェア開発に一番近いモデルで開発を行っていると考えられたためです。建築や製造業のプロジェクト管理手法をソフトウェア開発に適用したのが、最初のソフトウェア開発手法であるウォーターフォール型開発モデルです。

なぜ、これらのプロジェクト管理手法がソフトウェア開発の管理手法として適した手法であると考えられたのでしょう。建設分野・製造分野（特にオーダーメイド品生産）の開発管理手法は、「毎回異なるものを作り、前と丸々同じものを作ることはほぼない」という点でソフトウェア開発手法と非常に近しかったためです。当時現役のITエンジニアだった諸先輩にこの頃の話を聞くと、「開発管理手法が確立していなかった当時のソフトウェア開発（およびそのプロジェクト管理手法）からすると、非常に革新的だった」といいます。
...
//footnote[piason1][『ソフトウェア工学：理論と実践』（桐原書店,2001）から引用]
...
しかし、これらの開発手法はおおむね「開発者は顧客が要求することを最初からすべて理解している」「開発者は顧客にこの先要求されることを最初からすべて予測している」といった@<b>{前提が成立すればうまくいく}、@<b>{これからの変化にも耐えられる}という点にもとづいており、「最終の完成形のソフトウェアを作りきればその後はいじらない（いじりたくない）」という認識に立ちます。言い換えれば、@<b>{仕様書が過不足なく完璧に作られている}といった、現実的には不可能なことが必要条件となっています。しかし、未来は予測できない不確実なものです。

これらの従来型の開発手法でも、一応状況の変化などは考慮しています。ただし、その考え方は「状況の合わない部分が出ることはあって、そのときに小規模な改修はするが、基本は大きく合わなくなれば丸ごと作り直せばよい」という考えに立つものです。
...
さらに、1993年にJeff Sutherland、John Scumniotales、Jeff McKennaの3名がソフトウェア開発に適用した設計・分析ツールを構築しました。また、ときを同じくして、Ken Schwaberも自社でのソフトウェア開発にこの手法、@<b>{スクラム}を用いました。

その後、これらの取り組みが1995年のOOPSLAカンファレンスで共同発表されます。この内容はのちに『アジャイルソフトウェア開発スクラム』（桐原書店,2003）という書籍としてまとめられたことで、広く知られる事になります。
英語板は『Agile Software Development with SCRUM（Series in Agile Software Development）』（Prentice Hall,2001）
...
XPはKent Beck、Ward Cunningham によって生み出され、その後1996年にKent Beck は Ron Jeffries、Martin Fowler、Ron Jeffriesとともに、プロジェクトでXPの実践を行いました。
...
//footnote[xpbook][以下、XP本と記載します。邦訳の第2版は、現在も購入可能です（https://www.ohmsha.co.jp/book/9784274217623/）。また、前述したXPが初めて実践されたプロジェクトについての話も、第17章「はじまりの物語」に記載があります。]
...
このため、アジャイルを導入した最初の頃などは、「意図してアジャイルを進める」「アジャイルに関するロール・アクションを行う」時期が必要な場合もありますが、本来のありようとしては、「アジャイルである」という状態を指すものです。「アジャイルをする」ではなく、「（その開発手法が）アジャイルである」という状態に到達した状態が本来の姿で、正しいということになります。
...
XPについては、2001年頃から情報処理学会の刊行物（会誌、論文誌）で情報が出始め、2002年3月@<fn>{xpreport1}、4月の会誌@<fn>{xpreport2}で、XPについてのまとまった記事が掲載されたことが契機のようです。
...
//footnote[scrumreport][藤井拓,鶴原谷雅幸,大津尚史（2004）「普通のプロジェクトへの適用を目指したアジャイルな開発手法の構築と適用結果」情報処理学会研究報告Vol87,2004-SE-145,15-21（http://id.nii.ac.jp/1001/00021293/）]


当時の雑誌媒体についても、2誌で情報を調べてみました。おおよそこの歴史を裏づける内容で、情報が出てきたのも同様の順でした。

 * @<b>{XP}

Software Design誌

2002年09月号の特別企画「これでわかった！ XPの使い方」で一般的な記事が出ました。

2002年8月号からエクストリームプログラミングについての記載が出始め、

WebDBPress誌Vol.5（2001/11/15）の「PHPこども電話相談室」が最初です@<fn>{WebDBPress}。

//footnote[WebDBPress][ただし、Unit Test（単体テスト）のためのクラス「PHPUnit」を取り上げたものであり、あまりガッツリした記事はしばらくありません。]

 * @<b>{スクラム}

Software Design誌

2005年08月号の特集記事「緊急レポート！ 最新ソフトウェア開発手法事情」の中で、「4章　繰り返し型開発の落とし穴―失敗から学ぶ効果的運」が最初でした。とはいえ、あまりガッツリした内容ではありませんでした。
...
結局XPは当時（2002～2003年ぐらい）としてはあまりに先進的すぎたため、いろいろなアプローチがバズワード化してしまったり……と、それはそれで当時はネタにもされたものではありました（特にSI）。
...
XP本は、オーム社から第2版の翻訳が『エクストリームプログラミング』（2015）として出ています。
