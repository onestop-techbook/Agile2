...
例えば、プロジェクトマネジメントの知識体系ガイドである「プロジェクトマネジメント知識体系ガイド（PMBOK(R)ガイド）」@<fn>{pmbok}を見てみると、2017年に発行された第6版まではウォーターフォール型開発のような開発モデル@<fn>{pm6thmodel}が主たるモデルとされていましたが、ここで始めて「アジャイル」の内容がはじめて本文に組み込まれました。
...
ソフトウェア開発の黎明期（1970年頃）は国内外を問わず、情報システム産業全体の課題として、ソフトウェア開発の生産性やソフトウェア品質が非常に悪いことが大きな問題となっていました。この問題に対しては、さまざまな開発手法が提案されました。具体的には、ウォーターフォール型開発モデルや、構造化プログラミングといった、現在でも用いられている手法が提案され、さまざまなプロジェクトで実践されていきました。
...
※モデルの各プロセスの名称は SEC BOOKS 共通フレーム 2013,独立行政法人 情報処理推進機構 技術本部 ソフトウェア高信頼化センター,2013 から引用
...
これらの開発手法はおおむね「開発者は顧客が要求することを最初からすべて理解している」「開発者は顧客にこの先要求されることを最初からすべて予測している」といった@<b>{前提が成立すればうまくいく}、@<b>{これからの世の中の変化にも耐えられる}という点にもとづいており、「最終の完成形のソフトウェアを作りきればその後はいじらない（いじりたくない）」という認識に立ちます。言い換えれば、@<b>{仕様書が過不足なく完璧に作られている}といった、現実的には不可能なことが必要条件となっています。しかし、未来は予測できない不確実なものです。
...
これは、1986年に野中郁次郎博士と竹内弘高博士が、当時の日本の製造業における革新的な開発手法を分析し、「スクラム」と名づけて発表した論文@<fn>{scrunletter}をもとに開発されました。
...
なお今回、改めて情報処理学会の電子資料や当時刊行されていた雑誌２誌(SoftWareDesign誌、WEB+DBPRESS誌)を調べてみましたが、これを裏付けるものでした。
...
Software Design誌で最初にXPという単語の掲載が確認できたのは、2002年8月号の「実践プログラミング講座 コードデザイン最前線 vol.04」でした。

またこの翌月である2002年9月号には、特集として『特別企画「これでわかった！ XPの使い方」』が20ページに渡り掲載されており、当時の勢いが伺えます。

 * @<b>{WEB+DBPRESS誌}

WEB+DBPRESS誌で最初にXPという単語の掲載が確認できたのはWEB+DBPRESS誌Vol.5（2001/11/15）の「PHPこども電話相談室」でした@<fn>{WebDBPressXP}。
...
Software Design誌で最初にスクラムに関する情報が記載されたのは、2005年8月号の特集記事「緊急レポート！ 最新ソフトウェア開発手法事情」のなかで、「4章　繰り返し型開発の落とし穴―失敗から学ぶ効果的運」が最初の記載でした。とはいえ、あまりスクラムについて深堀りした内容ではありませんでした。

 * @<b>{WEB+DBPRESS誌}

WEB+DBPRESS誌で最初にスクラムに関する情報が記載されたのは、WEB+DBPRESS Vol.44(2008年5月号)の「特集3 オブジェクト指向開発の本質 設計、要求、開発プロセス」の「第4章 開発プロセスの本質 ウォーターフォールから反復・アジャイル開発プロセスへ」ですが、あくまで反復開発・アジャイル開発を横断的に触れたもので、スクラムについてもここで始めて言及がありました。
...
結局XPは当時（2002～2003年ぐらい）としてはあまりに先進的すぎたため、いろいろなアプローチがバズワード化してしまったり……と、それはそれで当時はネタにもされたものではありました（特に、当時筆者が所属していたSIの界隈だと）。
