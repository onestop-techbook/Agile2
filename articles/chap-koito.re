 ...
 * 内容：
 ** チームが開発するプロダクトに競合製品がある場合は、その製品をみんなで触って意見を交換する時間を設けました。みんなで画面を操作し気づいたことを言い合い、
 自分たちのプロダクトの価値向上につながるものはPBIに追加していくというものです。
 * 効果：
 ** POとチームが対等な立場で意見を交換できるので距離が縮まります。
 ** みんなで意見を共有することでチームの一体感が強くなります。
 ** POがビジョンを共有しやすくなります。
 ** サービスへの理解が深まり、チーム内の認識齟齬が減ります。

==== やってよかったこと3：PBIをチームみんなで作る
 * 内容：
 ** 私たちのチームでは、立ち上げ時こそユーザストーリーマッピングをみんなで検討したのですが、それ以降はなんとなくPOがユーザストーリーを考える人、開発者が開発する人というように、役割が決められてしまいました。その結果徐々にPOと開発者の関係が遠くなっていくのを感じたので、POにはPBIの完成度の低い（エピック単位までしか検討していない）状態でリファインメントに挑んでもらうことにしました。
