 ...
 * 内容：
 ** 私たちのチームでは、立上げ時こそユーザストーリーマッピングを皆で検討したのですが、それ以降は何となくPOがユーザストーリーを考える人、開発者が開発する人というように、役割が決められてしまいました。その結果徐々にPOと開発者の関係が遠くなっていくのを感じたので、POにはPBIの完成度の低い(エピック単位までしか検討していない)状態でリファインメントに挑んでもらうことにしました。
 ...
 * 内容：
 ** ステークホルダーの関心事を整理した後、ステークホルダーの参加するイベント/参加しなくてもよいイベントを決定しました@<fn>[comment]。
 ** 図10.3がイメージです。適切なスクラムイベントがない場合は、スクラムイベントに無理に参加してもらうわけではなく別の場を設けました。
