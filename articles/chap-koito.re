...
プロダクトを成功に導くために、上記のように多くの関係が必要となります。
 ...
 * 内容：
 ** 私たちのチームでは立上げ時こそユーザストーリーマッピングを皆で検討したのですが、それ以降は何となくPOがユーザストーリーを考える人、開発者が開発する人みたいな分類になっていました。
 ** 徐々にPOと開発者の関係が遠くなっていくのを感じたので、POにはPBIの完成度の低い(エピック単位までしか検討していない)状態でリファインメントに挑んでもらうことにしました。
 ...
 ** 休んだメンバが翌日以降第三者視点でおかしな点に気付き指摘することが出来ます。

==== やってよかったこと7：毎スプリント小さな変化を入れる
 * 内容：
 ** 同じ状況が続くと、チームのメンバは飽きてきてしまうので毎スプリントチームに変化を与えてみました。
 ** これまでやってみた内容
 *** メンバの入れ替え(SM⇆開発者の入替、複数チームであればチーム間でSMの入替)
 ...
 * 内容：
 ** ステークホルダーの関心事を整理した後、ステークホルダーの参加するイベントを決定しました。
 ** 以下がイメージです。適切なスクラムイベントがない場合は、スクラムイベントに無理に参加してもらうわけではなく別の場を設けました。
 ...
 *** もちろん、スプリントレビューなどのイベントには誰でも参加して良いのですが、初期はステークホルダー自身がチームとどう関わっていいか判断に悩むことが多いため、
 こちらから必須のイベントを指定する方式としました。
 ...
 *** スプリントが始まる前のチームビルディング：ステークホルダーや開発者を雑談（おやつ神社など）に連れ出し、メンバ間の顔合わせや価値感を探ります。
 ...
 * 内容：
 ** スプリントレビューはプロダクトの検査と適応を行う場です。スプリントレビューに対するステークホルダーの熱量を高めるために以下の工夫をしました。
 ** ワクワクするシナリオの検討
 ...
 * 内容：
 ** 「やってよかったこと10 スタートダッシュを成功させる」と反対の話になりますが、上手くいっていない状態も積極的に共有することでステークホルダーに味方になってもらい関係性を良くすることができます。
