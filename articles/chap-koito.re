...
最初から成果（@<strong>{結果の質}）を求めようとすると、チームが直接的な数字作りに走り、チーム内に摩擦や対立が生まれ（@<strong>{関係の質}の悪化）、
心理的安全性が確保されず（@<strong>{思考の質}の悪化）、行動が消極的になり（@<strong>{行動の質}の悪化）、さらに結果が悪くなる（@<strong>{結果の質}の悪化）
という「@<strong>{失敗の循環}」が生まれると述べられています。

反対に、「@<strong>{成功の循環}」としてチームの関係性がよくなることから着手する（@<strong>{関係の質}の向上）と、
 ...
 * やったこと
 ...
 さらにそのなかで、みんなで画面を操作し、気づいたことを言い合い、自分たちのプロダクトの価値向上につながるものは
 ...
 * やったこと
 ...
 あるチームでは、立上げ時こそユーザストーリーマッピングをみんなで検討しましたが、
 ...
 また、バックログリファインメントの中で、チーム全員でエピック単位のユーザストーリーマッピングを作成するワークを行いました。
 ...
 * 実践のコツ
 ...
 また、失敗した時の振る舞いも重要です。
 ふりかえりでネガティブな雰囲気にならないようよい雰囲気を作り、
 前向きな議論が進められるようにします。

==== やってよかったこと5：エンドユーザがサービスを使ってもらっているところを見る
 * やったこと：
 
 サービスイン後、実際にエンドユーザがサービスを使っているところを関係者と見学し、
 ...
 * やったこと：
 ...
 チームメンバーがあとからJoinした場合や、ベテランと若手など開発経験に差がある場合などは、
 チーム内に知識差が生まれてしまうケースがあります。

  この状態が続くと、質問する人／される人が固定化され、上下関係が生まれてしまいます。
 そこで、チームメンバー（特に有識者）がスクラムイベントを欠席することで、
 特定の個人がすべてを把握している状態を強制的になくし、
 ...
 * やったこと：
 ...
 変化のない環境でスプリントを進めていくと、安定はする一方で、マンネリ化しまうので、
...
ステークホルダーの関心事を一覧化し、可視化しました。
 ...
 誰が何に興味を持っているのかを整理し、その関心内容に応じてをチームから適切な関係性を築いていくことが重要です。
 例えば、〓図番号〓のように一覧に整理します。
 ...
 * 実践のコツ：
 ...
 フィードバックを出しにくくなる経験をしました。
 それ以降は初期はあえて参加者を絞ることで、より質の高いフィードバックをもらうようにしています。
 ...
 * やったこと：
 ...
 開発がスタートし、早い段階から動くプロダクトを見せないと、
「アジャイルっていっても早くないじゃん」と、ネガティブな印象が払拭されず、
 そこから関係性を作っていくのは非常に難しいものとなります。
 キックオフから最初の1ヵ月で、そこで動くプロダクトを見せられるかどうかが
 ポイントかなと私は考えており、1ヵ月で動くプロダクトを見せられるようベストを尽くします。

 * 効果：
 ** ステークホルダーがチーム、プロダクトに対してポジティブな感情になる
 ** 上記の結果、チームの活動に協力的になり以降の開発がしやすくなる

 * 実践のコツ：

 キックオフより前でもできることは進めておきます。
 ...
 * やったこと：
 ...
 スプリントレビューはプロダクトの検査と適応を行う場です。
 より質の高いフィードバックを得るために、まずはステークホルダーに
 スプリントレビューを好きになってもらうよう工夫をしました。
 ...
 ** レビュー時間以外でもプロダクトに対する気づきを共有するようになる

 * 実践のコツ：

 私の過去の失敗談です。
 主催するチームから見てもあまり楽しくないスプリントレビューを続けていたら
 ステークホルダーのなかでスプリントレビュー参加より他の打ち合わせを優先し、欠席することが増えてしまいました。
 なにで、ステークホルダーにレビューに参加する価値がある、そもそも参加自体が楽しいと思ってもらえるよう
 できる工夫をしていくことが大切です。

==== やってよかったこと12：うまくいってない状態を早期に曝け出す
 * やったこと：

「やってよかったこと10：スタートダッシュを成功させる」と反対の話になりますが、
 うまくいっていない状態も積極的に共有することで、ステークホルダーに味方になってもらい、
 関係性をよくすることができます。
 開発が順調にいってない場合、その事実を隠したくなる気持ちもわかりますが、
 逆に関係性を強めるチャンスと考え、積極的に開示します。
 スプリントゴールの未達や、リリースバーンダウンの芳しくない状況、
 チームが抱えている課題などの透明性を高めるだけでなく、
 それに対して議論する場を作り、チームと一緒に考えられる環境を作っていきます。
