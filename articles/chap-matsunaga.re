...
言葉で言い換えるなら、価値を見定めつつ、定期的にインクリメントをリリースしてフィードバックを獲得し、それを反映してさらなる価値を付加したインクリメントをリリースし、これを繰り返す、@<b>{つまり反復することでプロダクトの価値を高め続ける}（図２）、これがアジャイルの価値であり、原則であると言っているわけです。

//image[image2][従来開発型とアジャイル開発における価値の推移][scale=1.0]

一方でたとえばウォーターフォールのような従来型の進め方では、開発が進むにつれて価値が高まっていきますが、その間にも新しいテクノロジーの発明や目新しいUXの創造などにより市場はどんどん変化していくため、プロダクトの価値は徐々に劣化していくことになります（図2左）。

== 「反復」とは何か
人間は「反復」によって多くの技能を獲得します。スポーツでも習字でも音楽でもそうですが、反復練習なくして上手になることはありません。そして、反復周期は短いほど、つまり頻度が高いほど一般に練習の効果は高いです。例えば年に1回ピアノを弾いたってなかなか上達はしません。しかし、週に1回、できれば毎日練習すれば上達はずいぶん早くなることは想像に難くありません。

そして、アジャイル開発においては、反復がうまく行っていることを示す最も重要な指標は、毎回のイテレーション（スプリント）でチームがなんらかの価値あるインクリメントを完成させていることです。ソフトウェア開発においては、価値あるインクリメントとは@<b>{デモができるインクリメント}のことです。逆に、顧客への実動作デモができないなら、それは顧客価値があるとは言えません。なぜなら、実際に動作するところを確認できないと、顧客はそのソフトウェアに価値があるかどうかの判断ができないからです。また、価値判断が出来ないということはフィードバックを返すことができないということでもあり、つまりここでフィードバックループは途切れてしまい、図1や図2のような、アジャイルの重要な特徴の実現が難しくなってしまうのです。
...
この分割方法は、よくケーキにもたとえられます。よく知られたメタファなのでここでは解説しません（文字数）。ご存知ない方はググってみてください。すぐにたくさんの記事が見つかると思います。

そうやって分割されたプロダクトバックログが実際に作られ、リリースされることで顧客やユーザーは実際にそれを使ってみることができるようになり、使ってみて初めてフィードバックを返すことができるようになるわけです。図３のようなやり方だと、開発序盤から中盤までは、例えば要件定義書や設計書を見せるのが関の山で、これでは顧客からのフィードバックを得るのはほとんど不可能です。そりゃそうですよね、例えばカメラの要件定義書を見せて、このカメラ使いやすいですか？画質はどうですか？などと聞かれたって答えようがありません。
