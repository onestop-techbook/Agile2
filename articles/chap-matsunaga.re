...
また、同じくアジャイル宣言12の原則には下記のような記述もあります。
...
今や2-3ヶ月を「短い時間間隔」と言ってしまうのは議論ありそうですが、そこは20年も前の話ですので置いておくとして、つまりここに書かれていることは、図に示すと下記のようなイメージです。

//image[image1][アジャイルの代表的なプロセス][scale=1.0]

言葉で言い換えるなら、価値を見定めつつ、定期的にインクリメントをリリースしてフィードバックを獲得し、それを反映してさらなる価値を付加したインクリメントをリリースし、これを繰り返す。@<b>{つまり反復することでプロダクトの価値を高め続けること}（図２）、これがアジャイルの価値であり、原則であると言っているわけです。
...
この方法では反復はまったく機能していません。例えばゴルフのスイングを練習するには、自分のスイングを動画に撮って、１回～数回振るごとにビデオをチェックし、良くないところを直してまたクラブを振る、というフィードバックループを繰り返すことが効果的なのですが、図12.3の方法では、そのループが回っていません。つまり、図12.3のような状態は、何かを習得するには非常に不利な状態と言えます。
...
この状態に陥っているチームは、プロダクト開発そのもの以外もうまく行きません。なぜなら、上述したとおり人間は反復によって技能を習得するからです。例えば、心理的安全性にしても、自己組織化にしても、Ｔ字型スキルにしても、「反復（イテレーション）」をする中で徐々に習得されていくものだからです。
...
この分割方法は、よくケーキにも例えられます。よく知られたメタファなので、ここでは解説しません。ご存知ない方は調べててみてください。すぐにたくさんの記事が見つかると思います。

そうして分割されたプロダクトバックログが実際に作られ、リリースされることで顧客やユーザーは実際にそれを使ってみることができるようになり、使ってみて初めてフィードバックを返すことができるようになるわけです。図12.3のようなやり方だと、開発序盤から中盤までは、例えば要件定義書や設計書を見せるのが関の山で、これでは顧客からのフィードバックを得るのはほとんど不可能です。そりゃそうですよね、例えばカメラの要件定義書を見せて、「このカメラ使いやすいですか？」「画質はどうですか？」などと聞かれたって答えようがありません。
...
この図ではプロダクトを一旦小さなプロダクト（＝価値のあるもの）に分解しています。それは小さいが故にシンプルで、すなわち早く完成でき、早くリリースでき、その価値はすぐにフィードバックを得ることができるのです。必要があれば、これらの小さなプロダクト（バックログアイテム）をさらに小さなタスク(＝作業)に分解しても良いでしょう。
...
実は、プロダクトバックログづくりは初めてアジャイルに取り組むチームが最初にぶつかる壁のひとつです。図12.4に示したアンチパターン以外にも、プロダクトバックログアイテムのサイズが大きすぎて、完成までに１ヶ月～数ヶ月かかるというのもよく見かけます。インクリメントが付加する価値は、上述したように「ボタンを押すとランプが点く」ぐらいの、本当に小さなもので良いのです。試してみてください。
