...
言葉で言い換えるなら、価値を見定めつつ、定期的にインクリメントをリリースしてフィードバックを獲得し、それを反映してさらなる価値を付加したインクリメントをリリースし、これを繰り返す。@<b>{つまり反復することでプロダクトの価値を高め続けること}（〓図2〓）、これがアジャイルの価値であり、原則であるといっているわけです。
...
一応イテレーション（スプリント）は一定周期で区切っているし、スクラムイベントも定期的にやってる（たぶん）のですが、聡明な本書の読者ならすぐにお気づきなのではと思いますが、これはアジャイルではないですよね。はい、おっしゃるとおりです。これは「アジャイルに見える」だけの単なるウォーターフォールですね。プロダクトのリリースは最後の1回だけですし、フィードバックなんてどこにも取り込んでないし、価値を高めてもいない。でも残念ながらこういう似非アジャイルが巷にあふれているのは事実です。

この方法では反復はまったく機能していません。例えばゴルフのスイングを練習するには、自分のスイングを動画に撮って、1回～数回振るごとにビデオをチェックし、よくないところを直してまたクラブを振る、というフィードバックループを繰り返すことが効果的なのですが、〓図12.3〓の方法では、そのループが回っていません。つまり、〓図12.3〓のような状態は、何かを習得するには非常に不利な状態といえます。
...
プロダクトバックログは、その名のとおり「プロダクト」の「バックログ」です。何がいいたいかというと、プロダクトバックログアイテム（PBI）は、プロダクトを分解したものである必要があるということです。プロダクトバックログアイテムは、そのひとつずつがなんらかの「価値」を顧客やユーザーに提供できなくてはなりません。「価値」というのは、例えばユーザーが意識的あるいは無意識的に抱えているなんらかの問題を解決するとか、新しいことができるようになるとか、今まで以上にお金が儲かるようになるとか、そういうことです。
...
前述のような似非アジャイルになっているチームのバックログを見てみると、バックログアイテムは、例えば次のようになっていたりします。
...
これを図に示すと〓図番号〓のようになります。つまり、プロダクトをいきなりタスクに分解しているわけです。前述のとおり、タスクはそれ単体では価値とはならないため、顧客にリリースすることはできません。これは単なるToDoリストであり、プロダクトバックログではありません。このような作り方をすると、すべてのタスクが完了するまでプロダクトはリリース出来ません。こういう開発のやり方をなんと呼ぶかというと、そうです、ウォーターフォールと呼ぶわけですね。
...
これらは、機能としてはとても小さなものにすぎませんが、それ単体で十分な価値があります。通知がＯＮになっているかＯＦＦになっているかは、ユーザーにとってそれなりに重要な情報です。プロダクトバックログアイテム（PBI）は単体ではこのような@<b>{小さな価値}が提供できれば十分なのです。

これを図にすると〓図12.6〓のようになります。

この図ではプロダクトを一旦小さなプロダクト（＝価値のあるもの）に分解しています。それは小さいがゆえにシンプルで、すなわち早く完成でき、早くリリースでき、その価値はすぐにフィードバックを得ることができるのです。必要があれば、これらの小さなプロダクト（バックログアイテム）をさらに小さなタスク（＝作業）に分解してもよいでしょう。
...
実は、プロダクトバックログ作りは初めてアジャイルに取り組むチームが最初にぶつかる壁のひとつです。〓図12.4〓に示したアンチパターン以外にも、プロダクトバックログアイテムのサイズが大きすぎて、完成までに1ヵ月～数ヵ月かかるというのもよく見かけます。インクリメントが付加する価値は、前述したように「ボタンを押すとランプが点く」ぐらいの、本当に小さなものでよいのです。試してみてください。
