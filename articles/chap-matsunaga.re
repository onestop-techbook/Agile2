...
今や2-3ヶ月を「短い時間間隔」といってしまうのは議論ありそうですが、そこは20年も前の話ですので置いておくとして、つまりここに書かれていることは、図に示すと下記のようなイメージです。
...
この方法では反復はまったく機能していません。例えばゴルフのスイングを練習するには、自分のスイングを動画に撮って、１回～数回振るごとにビデオをチェックし、良くないところを直してまたクラブを振る、というフィードバックループを繰り返すことが効果的なのですが、図12.3の方法では、そのループが回っていません。つまり、図12.3のような状態は、何かを習得するには非常に不利な状態といえます。
