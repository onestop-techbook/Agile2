...
この章では、アジャイルの実践として、アジャイルであるために用いられるいくつかのフレームワークや手法を解説します。決してこれらがアジャイルにおけるすべてではありませんが、アジャイルを実践するうえで各手法が活用されるイメージの助けとなることを目的としています。また、陥りやすいポイントや重要なポイントを必要に応じて補足し、本書で得たキーワードをもとに実践できる姿勢を整えます。第一歩というよりは、歩きはじめる前に「よーいドン」の「よーい」をしてもらえるように執筆してます。
...
リーンキャンバスについて短く語ることは困難ですが、はじめて使用するときはぜひ枠を埋める順番に注意してください。枠が多いためどこから埋めたらよいかわからないといった疑問に対して、上で書かれている順番で作成することで埋めやすくなるでしょう。
...
スクラムイベントは、スクラムの作成物の検査と適応をするための公式の機会であり、このイベントを通して作成物を検査し、状況に適応します。各スクラムイベントは、複雑さを低減するために、同じタイミングと場所で開催されることが望ましいです。
（デイリースクラムは毎日10:00にタスクボードの前に集合、スプリントレビューは毎週水曜14:00−15:00で会議室⑥、など）


1つ目のイベント、スプリントとは、スクラムイベントの入れ物となるイベントのことです。これは固定期間で、定期的なリズムを作るのに用いられます。多くのプロダクト開発では１週間や2週間としてスプリントを用意しますが、1ヵ月以内の決まった長さであれば問題ありません。期間を短くすることで、複雑さを低減し、できる限り予測可能な範囲で計画を立てて実行し、ふりかえります。
