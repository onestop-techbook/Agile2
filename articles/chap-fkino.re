...
これ以外にも重要なドキュメントはあります。ドキュメントはWordやExcelで書かれた形式的なドキュメントに限りません。例えば、コミットログやPull Requestのコメントも立派なドキュメントです。これらには「なぜ変更したのか」、あるいは「なぜしなかったのか」を記載しておくことで、ソフトウェアのメンテナンスが格段にやりやすくなります。
...
『ビジョナリー・カンパニーZERO　ゼロから事業を生み出し、偉大で永続的な企業になる』（日経BP,2021）にはOPUR（One Person Ultimately Responsible：最終責任を負う者）を任命すべきだと書かれています。POはまさにOPURだといえます。一方で、OPURと同じぐらい重要なのが、誰もが隣人のために「歩道を雪かきする」文化があることだと同書には書かれています。自分の仕事に対して完全な責任を引き受けつつ、お互いの歩道の雪かきを助ける文化が大切だということです。
