...
ここに書いてあることが唯一の答え・正解というわけではありません。唯一の正解がないような質問をあえて選択しました。自分ならこう答える、自分たちのチームや組織ならこんな風にしたほうがよいと考えたり、これを元に対話をしたりする目的で活用していただけると嬉しいです。
...
前者（時間で見積もる）の問題点はベロシティの変化がわからなくなるという点です（時間で見積もっているのでベロシティは常に一定）。後者（規模で見積もる）ですとチームの改善が進んでベロシティが上がっているのか、それとも何かチームに課題があってベロシティが下がっているのかがわかります。
...
なんでも同じです。 スクラムも同じことです。スクラムができていることを目標にしてしまうといつまで経っても自信を持って「できています」といえるようにはならないでしょう。 目的にあわせて目指すべき達成度（目標）があるはずです。 目的にあわせて目標を自分たちで考えて決めていればよいと思います。
...
Agile Studio by 永和マネジメントシステムでは、「アジャイルカフェ@オンライン」として、視聴者の方から寄せられたアジャイルの悩みにこたえるオンラインイベントを定期的に開催しています。皆さんの悩みにアジャイルコーチが「あーでもない、こーでもない」「これならいけそうかも」と解決策を楽しく考えていきます。このイベントでは、みなさんからの悩みにも一緒になって考えていくので、ご参加をお待ちしています。
