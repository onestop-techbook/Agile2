...
A：ドキュメントとひとことにいってもさまざまな種類のドキュメントがあります。手はじめに現状作成しているドキュメントを次の3つに分類してみましょう。
...
チームが信頼関係をもって仕事をできるなら、合意事項をこと細かに文書化し訴訟リスクに対応するような、自己防衛のためのドキュメントも必要ないでしょう。そんなドキュメントを作っている時間があったら信頼関係を築くことに力を注いだほうが得です。
...
『ビジョナリー・カンパニー ZERO』には OPUR（One Person Ultimately Responsible：最終責任を負う者）を任命すべきだと書かれています。POはまさにOPURだといえます。一方で、OPURと同じぐらい重要なのが、誰もが隣人のために「歩道を雪かきする」文化があることだと同書には書かれています。自分の仕事に対して完全な責任を引き受けつつ、お互いの歩道の雪かきを助ける文化が大切だということです。

== 見積り（時間とポイント）に関する Q&A

@<strong>{Q：時間の見積りとポイントの見積りのちがいがよく分かりません。相対的に見積もったとしても結局、時間に換算するのではないでしょうか？}
...
例えば、ある仕事（仕事A）があったときにそれを「3ポイント」と見積もり、その仕事をこなすのに結果的に6時間かかったとします。 1ヵ月後、仕事Aと同じような仕事Bをやることになりました。仕事Aをやったことでコツが分かっていたので、2時間でできそうだと思ったとします。ポイントも3分の1に減らして「1ポイント」にしました。 これをやってしまうとベロシティはどうなるでしょうか？6時間でこなせる仕事は「3ポイント」分のまま変わりません。
...
スクラムは問題を明るみに出すためのフレームワークです。問題が分かるのはいいことです。もしも、問題があるのに問題がないかのように取り繕おうとするなら、最初からスクラムを使わないほうがいいでしょう。
...
　　→スクラムマスターが困りごとを個別に1on1を行い、信頼関係作りからはじめる（スクラムをやってもらう前に、その人の関心ごとや困りごとを聞いて、抱えている問題を一緒に解決するような動きをするとうまくいく場合が多い）


SMとして積極的でないメンバーの内面状況がまったく予想できないのであれば

　　→スクラムマスターが困りごとを個別に1on1を行い、信頼関係作りからはじめる
...
何でも同じです。 スクラムも同じことです。スクラムができていることを目標にしてしまうといつまで経っても自信を持って「できています」といえるようにはならないでしょう。 目的にあわせて目指すべき達成度（目標）があるはずです。 目的にあわせて目標を自分たちで考えて決めていればよいと思います。
