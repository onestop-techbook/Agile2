...
私と同じようにアジャイルに初めて向き合い、初めて悩んでいる方にとっての何かの参考になれば幸いです。

== 開発チームに新規着任

昨年、筆者はプロダクトのバッチ開発担当から別の新しい開発チームにアサインされました。5名のチームに対して、自分を含めた3名の新規着任、サブリーダー相当の方が1名離任するという規模の大きいメンバー入れ替えが発生したタイミングでの着任になります。
...
しかし、上司は「メンバーの大量入れ替えにともない、チームの文化とスタンス部分を心配している」といっていました。チームに特に問題を感じていない自分は、その言葉の真意を理解することができませんでした。なお、〓表14.1〓は、筆者の開発チームでの開発イベントの呼び方を、一般的なスクラムイベントの呼び方に対応させた表です。呼び方そのものは本質的なものではないかもしれませんが、その目的が必ずしも共有、一致していなかったのかもしれません。
...
この質問に答えられないということは、毎日行われている朝会は実施されているが、なんのために行っているのか誰もわからない恐ろしい状況であることを、このタイミングで初めて認識しました。

朝会は日々の作業内容を報告すればよいのかなとぼんやり思っていましたが、進捗状況を共有する理由、朝会では話すべき話題など、会議体として重要な項目が明確でないまま運用されている現状がそこにはありました。
...
== インセプションデッキとの出会い
このままではよくないことは理解できましたが、どうやって現状を打破できるか途方に暮れていました。そんな自分に対して、上司は「インセプションデッキ的な何かをチームで取り組んだほうがよいかもね」という助言をくれました。その助言を聞いた自分は「なるほど、わからん」状態でしたので、その言葉の意味を調べ、書籍「アジャイルサムライ-達人開発者への道」（オーム社、2011）にたどり着きました。
...
もし、自分と同じようになんとなく朝会など定期イベントは実施しているけど「なんでやっている？」という質問に答えられない、あるいはチームメンバーで回答バラバラな状況であれば、まずはインセプションデッキの「我々はなぜここにいるのか」だけでも時間をかけて考えることをおすすめします。チームの拠り所ができて、以降の議論がスムーズになると思います。
